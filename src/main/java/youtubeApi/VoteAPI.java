...
public class VoteAPI extends Thread{
    ...
    public static InsertLiveChatMessage in = new InsertLiveChatMessage();
    ...
    public void run(){
        ...
        try {
            ...
            in.InsertLiveChatMessage(liveChatId,voteMessage);
            detectTime = in.getPublishTime();
...
//        },5000L); //每5秒傳一次結果
        while(System.currentTimeMillis() < end){
            try {
                ...
                for (int i = 0; i < messages.size(); i++) {
                    ...
                    if(snippet.getPublishedAt().getValue() < detectTime.getValue()) notInTime = true;
        ...
        if(!voteResult.isEndFlag())
            in.InsertLiveChatMessage(liveChatId,"投票已結束");
        //timer.cancel();
            ...
            if(!notInTime){ //在投票時間內
                if(message.equals(keywords.get(i))){    //偵測每個投票關鍵字
                    if(voteResult.getVoteResult().get(author.getDisplayName()) == null){    //是否重複投票
                        ...
                        in.InsertLiveChatMessage(liveChatId,author.getDisplayName() + " 投了 " + voteOptions.get(i));
    ...
    public static void setVoteData(VoteData voteData) {
        VoteAPI.voteData = voteData;
    }

    public VoteResult getVoteResultData(){
        return voteResult;
    }
